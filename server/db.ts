...
import { InsertSavedScenario, InsertUser, savedScenarios, users } from "../drizzle/schema";
...
export async function createSavedScenario(userId: number, input: Omit<InsertSavedScenario, "id" | "userId" | "createdAt" | "updatedAt">) {
  const db = await getDb();
  if (!db) throw new Error("Scenario storage is temporarily unavailable");
  await db.insert(savedScenarios).values({ userId, ...input });
}

export async function deleteSavedScenario(userId: number, scenarioId: number) {
  const db = await getDb();
  if (!db) throw new Error("Scenario storage is temporarily unavailable");
  await db.delete(savedScenarios).where(and(eq(savedScenarios.id, scenarioId), eq(savedScenarios.userId, userId)));
}
