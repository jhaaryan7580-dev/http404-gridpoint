...
function MapPlot({
  ...
  const locate = (point: Point) => ({ x: 7 + ((point.lon - minLon) / lonSpan) * 86, y: 92 - ((point.lat - minLat) / latSpan) * 84 });
  ...
  const toPoint = (event: Pick<ReactPointerEvent<SVGSVGElement>, "clientX" | "clientY">): Point => {
    ...
    return { lon: minLon + ((x - 7) / 86) * lonSpan, lat: minLat + ((92 - y) / 84) * latSpan };
  };
  ...
  return <div className="map-shell">
    ...
    <svg ref={svgRef} className="network-svg" viewBox="0 0 100 100" preserveAspectRatio="none" aria-label="Demand nodes and proposed warehouse hubs" onPointerMove={(event) => { if (dragHubId !== null) onHubDrag(dragHubId, toPoint(event)); }} onPointerUp={onHubDragEnd} onPointerCancel={onHubDragEnd}>
      {showZones && hubs.map((hub) => {
        ...
        const rx = Math.min(26, (maxDistance / lonSpan / (EARTH_KM_PER_LAT * Math.cos(hub.lat * Math.PI / 180))) * 86 * 1.14);
        const ry = Math.min(26, (maxDistance / latSpan / EARTH_KM_PER_LAT) * 84 * 1.14);
      ...
      {radius && hubs.map((hub) => { const { x, y } = locate(hub); const radiusPercent = (radius / latSpan / EARTH_KM_PER_LAT) * 84; return <circle key={`ring-${hub.hubId}`} cx={x} cy={y} r={radiusPercent} className="radius-ring" style={{ stroke: PALETTE[hub.hubId % PALETTE.length] }} />; })}
...
export default function Home() {
  ...
  const exportAssignments = () => downloadCsv("gridpoint-assignments.csv", ["Neighborhood", "Lat", "Lon", "Orders", "Assigned Hub", "Distance to Hub", "Vehicle", "Rate per km/order", "Delivery Cost", "Service Status"], result.assignments.map((assignment) => [assignment.name, assignment.lat, assignment.lon, assignment.adjustedOrders, `H${assignment.hubId + 1}`, oneDecimal(assignment.distance), assignment.vehicle, assignment.distance && assignment.adjustedOrders ? oneDecimal(assignment.cost / assignment.distance / assignment.adjustedOrders) : 0, Math.round(assignment.cost), assignment.withinRadius ? "In range" : "Beyond range"]));
  ...
  const generateReport = () => {
    const rows = result.assignments.map((assignment) => `<tr><td>${escapeHtml(assignment.name)}</td><td>H${assignment.hubId + 1}</td><td>${oneDecimal(assignment.distance)} km</td><td>${assignment.adjustedOrders.toLocaleString()}</td></tr>`).join("");
    const reportHtml = `<!doctype html><html><head><meta charset="utf-8"><title>HTTP 404 decision report</title><style>body{font-family:Arial,sans-serif;color:#1d2b24;margin:42px}h1{font-size:30px;margin:0 0 6px}p{color:#64736a}.grid{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin:28px 0}.card{border:1px solid #dbe1da;border-radius:9px;padding:15px}.label{color:#778179;font-size:11px;text-transform:uppercase;letter-spacing:.08em}.value{font-size:23px;font-weight:700;margin-top:5px}table{width:100%;border-collapse:collapse;margin-top:22px}th,td{padding:9px;border-bottom:1px solid #dbe1da;text-align:left;font-size:12px}th{color:#778179;font-size:10px;text-transform:uppercase}.print-button{float:right;background:#1d2b24;color:#fff;border:0;border-radius:6px;padding:9px 13px;cursor:pointer}@media print{body{margin:24px}.print-button{display:none}}</style></head><body><button class="print-button" onclick="window.print()">Print / Save PDF</button><div class="label">HTTP 404 · DECISION REPORT</div><h1>${escapeHtml(dataset)} warehouse network</h1><p>${params.surge ? `Peak demand +${params.surge}%` : "Base demand"} · ${result.totalOrders.toLocaleString()} daily orders · ${result.hubs.length} hubs</p><section class="grid"><div class="card"><div class="label">Total cost</div><div class="value">${money(result.totalCost)}</div></div><div class="card"><div class="label">Modeled savings</div><div class="value">${oneDecimal(result.savingsPercent)}%</div></div><div class="card"><div class="label">Service level</div><div class="value">${oneDecimal(result.coverage)}%</div></div><div class="card"><div class="label">Delivery cost</div><div class="value">${money(result.deliveryCost)}</div></div><div class="card"><div class="label">Infrastructure</div><div class="value">${money(result.infraCost)}</div></div><div class="card"><div class="label">Weighted avg. leg</div><div class="value">${oneDecimal(result.weightedDistance)} km</div></div></section><h2>Assignment audit</h2><table><thead><tr><th>Neighborhood</th><th>Assigned hub</th><th>Distance</th><th>Orders</th></tr></thead><tbody>${rows}</tbody></table><p>Model assumptions: weighted k-means, local Euclidean distance, fixed cost per hub, and daily order volume as demand weight.</p></body></html>`;
    const reportUrl = URL.createObjectURL(new Blob([reportHtml], { type: "text/html;charset=utf-8" }));
    let reportWindow: Window | null = null;
    try { reportWindow = window.open(reportUrl, "_blank", "noopener,noreferrer"); } catch { reportWindow = null; }
    if (!reportWindow) {
      const link = document.createElement("a"); link.href = reportUrl; link.download = "http404-decision-report.html";
      document.body.appendChild(link); link.click(); link.remove();
      toast.info("Popup was blocked, so the report was downloaded instead.");
    } else {
      toast.success("Decision report generated — print or save it as PDF from the report tab.");
    }
  ...
  return <div className="site-shell" data-accent={accent}>
    <aside className="control-rail">
      ...
      <section className="rail-section">
        ...
        <select value={params.algorithm} onChange={(event) => { setManualHubs(null); updateParams({ algorithm: event.target.value as Algorithm }); }}><option value="kmeans">Optimal point · weighted k-means</option><option value="kmedoids">Existing node · snap-to-site placement</option></select>
      ...
      <section className="rail-section">
        ...
        {params.useRadius && <label className="radius-slider"><span>Max Service Radius (km)</span><b>{params.radius} km</b><input className="range-control" type="range" min="3" max="15" value={params.radius} onChange={(event) => updateParams({ radius: Number(event.target.value) })} /><small>3 km</small><small>15 km</small></label>}
      </section>
      <section className="rail-section">
